...
MEM_RMSD <- function(MEM_vals,format=NULL,newWindow.heatmaps=FALSE,output.matrix=FALSE){
    ...
    if(isTRUE(format=="pop files")){
        ...
        for(m in 1:length(infiles)){
            for(n in 1:length(infiles)){
                ...
                if(dim(popA1)[2]==2){
                    popA1 = read.table(paste(MEM_vals,"/",infiles[[m]],sep=""),row.names=1)
                }
                popB1 = read.table(paste(MEM_vals,"/",infiles[[n]],sep=""))
                if(dim(popB1)[2]==2){
                    popB1 = read.table(paste(MEM_vals,"/",infiles[[n]],sep=""),row.names=1)
                }
        ...
        RMSD = matrix(nrow=nrow(MEM_scores),ncol=nrow(MEM_scores))
        similarity = matrix(nrow=nrow(MEM_scores),ncol=nrow(MEM_scores))
    ...
    heat_palette <- colorRampPalette(c("blue","green","yellow","red"))
    scale_min = min(similarity)
    scale_max = max(similarity)
    scale_range = 100-scale_min

    pairs.breaks <- c(seq(scale_min,(scale_min+scale_range/4),by=0.01),seq((scale_min+scale_range/3.9),(scale_min+scale_range/2),by=0.01),seq((scale_min+scale_range/1.9),(scale_max-scale_range/3.9),by=0.01),seq((scale_max-scale_range/4),100,by=0.01))

    if(newWindow.heatmaps==TRUE){
        X11()
    }
    clustered <- heatmap.2(as.matrix(similarity),main = "MEM RMSD",dendrogram="both",trace="none",key=TRUE,col=heat_palette,breaks=pairs.breaks,margins=c(7,15))
    ...
    write.table(as.matrix(reorder_similarity),paste("./output files/",strftime(Sys.time(),"%Y-%m-%d_%H%M%S")," MEM_RMSD1.txt",sep=""),sep="\t",row.names=TRUE)}
