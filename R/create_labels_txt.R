create.labels.txt <- function(MEM_vals_scale, display.thresh,heatmap_data){
  ...
  for(i in 1:nrow(heatmap_data)){
    ...
    negMarkers = MEM_vals_scale[i,][MEM_vals_scale[i,]<0 & abs(MEM_vals_scale[i,])>=display.thresh]

    #     posMarkers = MEM_vals_scale[i,][1:3]
    #     negMarkers = MEM_vals_scale[i,][1:3]
    ...
    posSignedVec = as.matrix(paste(posVector,abs(round(posMarkersOrdered,2)),sep=""))
    ...
    negSignedVec = as.matrix(paste(negVector,abs(round(negMarkersOrdered,2)),sep=""))
  ...
  new_rownames = paste(row.names(MEM_vals_scale),":",'UP',posRownamesMatrix,paste(dot),'DN',negRownamesMatrix,sep=" ")
  return(new_rownames)
}
